...
t=$(../scripts/memguard.py --cap-mb "${OTTEROS_MEM_CAP_MB:-3072}" -- cargo test -q -p "$c" 2>&1 | awk '/^test result/{p+=$4; f+=$6; i+=$8} END{printf "tests %d passed %d failed %d ignored", p, f, i}')

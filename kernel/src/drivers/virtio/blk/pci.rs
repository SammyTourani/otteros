...
pub fn read_pci_u8(base: *const u8, ecam_offset: u32, cap_offset: u8, offset: usize) -> u8 {
    // SAFETY: base is a valid ECAM window mapped by map_ecam_window; ecam_offset and the computed
    // offset are within the PCI config space (0x0 - 0xff).
...
pub fn read_pci_u32(base: *const u8, ecam_offset: u32, cap_offset: u8, offset: usize) -> u32 {
    // SAFETY: base is a valid ECAM window mapped by map_ecam_window; ecam_offset and the computed
    // offset are within the PCI config space (0x0 - 0xff).
...
pub fn write_pci_u16(base: *const u8, ecam_offset: u32, offset: u32, value: u16) {
    // SAFETY: base is a valid ECAM window mapped by map_ecam_window; ecam_offset + offset
    // is within the PCI config space.

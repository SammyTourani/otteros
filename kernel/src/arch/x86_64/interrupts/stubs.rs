//! The mechanical per-vector stub table (brief M1-T1): `define_stub!`
//! generates one `#[unsafe(naked)] unsafe extern "C" fn` per CPU-exception/
//! IRQ vector that doesn't need special `swapgs` handling, all jumping to
//! `super::common_stub`. Split out of `interrupts.rs` purely to keep that
//! file under the ~600-line guideline -- there is no meaningful logic
//! here beyond "256 near-identical one-liners"; `super::common_stub`,
//! `common_stub_ist1`/`common_stub_ist2` and vectors 2/8/18's own hand-
//! written stubs (the vectors that actually need different `swapgs`
//! handling, see `interrupts.rs`'s module docs) all still live there.

use super::{stub_2, stub_8, stub_18};

//! SSE2 kernels (4-wide f32 SIMD).
//! CPUID checked before use; only reachable if sse2_supported() == true.
//!
//! All kernels wrap unsafe inner functions marked #[target_feature(enable = "sse2")].
//! SAFETY: Kernels::detect() only selects these after CPUID confirms SSE2 support.

#![allow(unsafe_code)]

use core::arch::x86_64::*;
use core::ops::Range;

// ===== BOUNDS CHECKING =====

fn check_bounds(w: &[f32], rows: usize, cols: usize, x: &[f32], out: &[f32]) {
    assert!(
        w.len() >= rows.saturating_mul(cols),
        "sse2 f32: w too small ({} < {}*{})",
        w.len(),
        rows,
        cols
    );
    assert!(
        x.len() >= cols,
        "sse2 f32: x too small ({} < {})",
        x.len(),
        cols
    );
    assert!(
        out.len() >= rows,
        "sse2 f32: out too small ({} < {})",
        out.len(),
        rows
    );
}

fn check_bounds_q8(w_blocks: &[u8], rows: usize, cols: usize, x: &[f32], out: &[f32]) {
    let needed_blocks = (rows.saturating_mul(cols).saturating_add(31)) / 32;
    let needed_bytes = needed_blocks.saturating_mul(36);
    assert!(
        w_blocks.len() >= needed_bytes,
        "sse2 q8: w_blocks too small ({} < {})",
        w_blocks.len(),
        needed_bytes
    );
    assert!(
        x.len() >= cols,
        "sse2 q8: x too small ({} < {})",
        x.len(),
        cols
    );
    assert!(
        out.len() >= rows,
        "sse2 q8: out too small ({} < {})",
        out.len(),
        rows
    );
}

// ===== DOT PRODUCT =====

#[inline]
#[target_feature(enable = "sse2")]
unsafe fn dot_f32_inner(a: &[f32], b: &[f32]) -> f32 {
    // SAFETY: the loop reads a[i..i+W] and b[i..i+W] only while i + W <= len = min(a.len(), b.len()),
    // and the scalar tail indexes below len; the target features are guaranteed by Kernels::detect().
    unsafe {
        let len = a.len().min(b.len());
        let mut acc = _mm_setzero_ps();

        let chunks = len / 4;
        for i in 0..chunks {
            let i4 = i * 4;
            let av = _mm_loadu_ps(a.as_ptr().add(i4));
            let bv = _mm_loadu_ps(b.as_ptr().add(i4));
            acc = _mm_add_ps(acc, _mm_mul_ps(av, bv));
        }

        let mut result = [0.0; 4];
        _mm_storeu_ps(result.as_mut_ptr(), acc);
        let hsum = result.iter().sum::<f32>();

        hsum + crate::scalar::dot_f32(&a[chunks * 4..], &b[chunks * 4..])
    }
}

pub fn dot_f32(a: &[f32], b: &[f32]) -> f32 {
    // SAFETY: Kernels::detect() has already verified SSE2 support via CPUID.
    unsafe { dot_f32_inner(a, b) }
}

// ===== F32 MATRIX-VECTOR =====

#[target_feature(enable = "sse2")]
unsafe fn matvec_f32_rows_inner(
    row_range: Range<usize>,
    w: &[f32],
    rows: usize,
    cols: usize,
    x: &[f32],
    out: &mut [f32],
) {
    // SAFETY: the caller checked w.len() >= rows*cols, x.len() >= cols and out.len() >= rows; every
    // pointer offset below is r*cols + c with r < rows and c + W <= cols (or c < cols in the tail).
    unsafe {
        let x_len = x.len().min(cols);

        for r in row_range {
            if r >= rows {
                break;
            }

            let row_start = r * cols;

            // Two 4-lane accumulators for latency hiding
            let mut acc0 = _mm_setzero_ps();
            let mut acc1 = _mm_setzero_ps();

            // Process 8 columns at a time
            let chunks8 = x_len / 8;
            for c8 in 0..chunks8 {
                let c = c8 * 8;
                let wv0 = _mm_loadu_ps(w.as_ptr().add(row_start + c));
                let xv0 = _mm_loadu_ps(x.as_ptr().add(c));
                acc0 = _mm_add_ps(acc0, _mm_mul_ps(wv0, xv0));

                let wv1 = _mm_loadu_ps(w.as_ptr().add(row_start + c + 4));
                let xv1 = _mm_loadu_ps(x.as_ptr().add(c + 4));
                acc1 = _mm_add_ps(acc1, _mm_mul_ps(wv1, xv1));
            }

            let sum_acc = _mm_add_ps(acc0, acc1);
            let mut sum_vals = [0.0; 4];
            _mm_storeu_ps(sum_vals.as_mut_ptr(), sum_acc);
            let mut sum = sum_vals.iter().sum::<f32>();

            // Scalar tail
            let tail_start = chunks8 * 8;
            for c in tail_start..x_len {
                sum += *w.get_unchecked(row_start + c) * *x.get_unchecked(c);
            }

            if r < out.len() {
                out[r] = sum;
            }
        }
    }
}

pub fn matvec_f32(w: &[f32], rows: usize, cols: usize, x: &[f32], out: &mut [f32]) {
    check_bounds(w, rows, cols, x, out);
    // SAFETY: Bounds checked above; SSE2 verified by Kernels::detect().
    unsafe { matvec_f32_rows_inner(0..rows, w, rows, cols, x, out) }
}

pub fn matvec_f32_rows(
    row_range: Range<usize>,
    w: &[f32],
    rows: usize,
    cols: usize,
    x: &[f32],
    out: &mut [f32],
) {
    check_bounds(w, rows, cols, x, out);
    // SAFETY: Bounds checked above; SSE2 verified by Kernels::detect().
    unsafe { matvec_f32_rows_inner(row_range, w, rows, cols, x, out) }
}

// ===== Q8_0 MATRIX-VECTOR =====

#[target_feature(enable = "sse2")]
unsafe fn matvec_q8_rows_inner(
    row_range: Range<usize>,
    w_blocks: &[u8],
    rows: usize,
    cols: usize,
    x: &[f32],
    out: &mut [f32],
) {
    // Same block-wise walk as the AVX2 kernel. SSE2 has no sign-extending byte loads, so int8 is
    // widened by pairing each byte with itself and shifting right arithmetically (8, then 16).
    let wp = w_blocks.as_ptr();
    let xp = x.as_ptr();
    for r in row_range {
        if r >= rows {
            break;
        }
        let start = r * cols;
        let end = start + cols;
        let mut acc = _mm_setzero_ps();
        let mut tail = 0.0f32;
        let mut idx = start;
        while idx < end {
            let lane = idx % 32;
            let n = (32 - lane).min(end - idx);
            // SAFETY: the caller checked w_blocks.len() >= ceil(rows*cols/32)*36 and x.len() >= cols;
            // idx < rows*cols, so block idx/32 and its 4-byte scale are in bounds, and the n weights
            // and x[idx-start .. idx-start+n] read below stay inside the block and inside x.
            unsafe {
                let block = wp.add((idx / 32) * 36);
                let scale = f32::from_le_bytes(core::ptr::read_unaligned(block.add(32) as *const [u8; 4]));
                let xs = xp.add(idx - start);
                if n == 32 {
                    let mut b0 = _mm_setzero_ps();
                    let mut b1 = _mm_setzero_ps();
                    for half in 0..2 {
                        let v = _mm_loadu_si128(block.add(half * 16) as *const __m128i);
                        let lo16 = _mm_srai_epi16(_mm_unpacklo_epi8(v, v), 8);
                        let hi16 = _mm_srai_epi16(_mm_unpackhi_epi8(v, v), 8);
                        let f0 = _mm_cvtepi32_ps(_mm_srai_epi32(_mm_unpacklo_epi16(lo16, lo16), 16));
                        let f1 = _mm_cvtepi32_ps(_mm_srai_epi32(_mm_unpackhi_epi16(lo16, lo16), 16));
                        let f2 = _mm_cvtepi32_ps(_mm_srai_epi32(_mm_unpacklo_epi16(hi16, hi16), 16));
                        let f3 = _mm_cvtepi32_ps(_mm_srai_epi32(_mm_unpackhi_epi16(hi16, hi16), 16));
                        let xh = xs.add(half * 16);
                        b0 = _mm_add_ps(b0, _mm_mul_ps(f0, _mm_loadu_ps(xh)));
                        b1 = _mm_add_ps(b1, _mm_mul_ps(f1, _mm_loadu_ps(xh.add(4))));
                        b0 = _mm_add_ps(b0, _mm_mul_ps(f2, _mm_loadu_ps(xh.add(8))));
                        b1 = _mm_add_ps(b1, _mm_mul_ps(f3, _mm_loadu_ps(xh.add(12))));
                    }
                    acc = _mm_add_ps(acc, _mm_mul_ps(_mm_add_ps(b0, b1), _mm_set1_ps(scale)));
                } else {
                    let mut s = 0.0f32;
                    for i in 0..n {
                        s += (*block.add(lane + i) as i8) as f32 * *xs.add(i);
                    }
                    tail += s * scale;
                }
            }
            idx += n;
        }
        out[r] = hsum128_ps(acc) + tail;
    }
}

#[target_feature(enable = "sse2")]
fn hsum128_ps(v: __m128) -> f32 {
    let s = _mm_add_ps(v, _mm_movehl_ps(v, v));
    _mm_cvtss_f32(_mm_add_ss(s, _mm_shuffle_ps(s, s, 1)))
}

pub fn matvec_q8(w_blocks: &[u8], rows: usize, cols: usize, x: &[f32], out: &mut [f32]) {
    check_bounds_q8(w_blocks, rows, cols, x, out);
    // SAFETY: Bounds checked above; SSE2 verified by Kernels::detect().
    unsafe { matvec_q8_rows_inner(0..rows, w_blocks, rows, cols, x, out) }
}

pub fn matvec_q8_rows(
    row_range: Range<usize>,
    w_blocks: &[u8],
    rows: usize,
    cols: usize,
    x: &[f32],
    out: &mut [f32],
) {
    check_bounds_q8(w_blocks, rows, cols, x, out);
    // SAFETY: Bounds checked above; SSE2 verified by Kernels::detect().
    unsafe { matvec_q8_rows_inner(row_range, w_blocks, rows, cols, x, out) }
}

// ===== HELPER FUNCTIONS =====

pub fn rmsnorm(x: &mut [f32], eps: f32) {
    if x.is_empty() {
        return;
    }
    // Use scalar implementation for now
    crate::scalar::rmsnorm(x, eps);
}

pub fn axpy(alpha: f32, x: &[f32], y: &mut [f32]) {
    let len = x.len().min(y.len());
    let alpha_vec = unsafe { _mm_set1_ps(alpha) };

    let chunks = len / 4;
    for i in 0..chunks {
        let i4 = i * 4;
        // SAFETY: Bounds checked by len calculation.
        let xv = unsafe { _mm_loadu_ps(x.as_ptr().add(i4)) };
        let yv = unsafe { _mm_loadu_ps(y.as_ptr().add(i4)) };
        let prod = unsafe { _mm_mul_ps(alpha_vec, xv) };
        let result = unsafe { _mm_add_ps(prod, yv) };
        unsafe { _mm_storeu_ps(y.as_mut_ptr().add(i4), result) };
    }

    for i in (chunks * 4)..len {
        y[i] += alpha * x[i];
    }
}

pub fn softmax_inplace(x: &mut [f32]) {
    if x.is_empty() {
        return;
    }
    // Use scalar implementation for now
    crate::scalar::softmax_inplace(x);
}

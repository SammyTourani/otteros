//! Project Wycheproof's `ecdsa_secp384r1_sha384_test` vectors
//! (`scripts/gen-crypto-vectors.py`'s `ecdsa_p384_sha384_wycheproof.txt`).
//! Same shape as `ecdsa_p256_wycheproof.rs`; see that file's doc comment.

mod common;

use common::{hex_decode, read_fixture};
use otter_crypto::p384::PublicKey;
use otter_crypto::sha512::Sha384;

#[test]
fn ecdsa_p384_sha384_wycheproof() {
    let rows = read_fixture("ecdsa_p384_sha384_wycheproof.txt");
